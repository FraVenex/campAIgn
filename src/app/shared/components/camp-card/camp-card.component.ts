...
@Component({
	...
	template: `
		<div
			class="camp-card relative overflow-hidden transition-all duration-300 w-full"
			[ngClass]="{
				'h-full flex flex-col justify-between': fullHeight(),
			...
			@if (title() || subtitle() || icon()) {
				<div class="flex items-center justify-between mb-6 relative z-10">
					<div class="flex items-center gap-3">
						@if (icon()) {
							<div
								class="w-10 h-10 rounded-full flex items-center justify-center border shadow-xs transition-transform group-hover:scale-110 shrink-0"
								[ngClass]="{
						...
						<div>
							...
							@if (title()) {
								<h3
									class="text-xl font-serif tracking-tight leading-tight"
